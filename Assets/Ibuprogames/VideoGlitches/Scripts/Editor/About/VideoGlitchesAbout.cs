...
namespace Ibuprogames
{
  namespace VideoGlitchesAsset
  {
    ...
    /// About window.
    /// </summary>
    public sealed class VideoGlitchesAbout : EditorWindow
    {
      public static readonly string assetID = @"Ibuprogames.VideoGlitches";
      private const string description = @"A collection of post-processing effects for Unity that simulate several common failures in television and video signals, both analog and digital.";

      private const string documentationURL = @"http://www.ibuprogames.com/2015/07/03/video-glitches/";
      private const string publisherURL = @"https://assetstore.unity.com/publishers/8484";
      private const string forumURL = @"https://forum.unity.com/threads/released-video-glitches.338251/";
      private const string storeURL = @"https://www.assetstore.unity3d.com/#!/content/40474";
      ...
      [MenuItem("Help/Ibuprogames/Video Glitches/About", false, 0)]
      public static void MenuAbout()
      {
        Open();
      }

      [MenuItem("Help/Ibuprogames/Video Glitches/Documentation", false, 0)]
      ...
      {
        string[] results = AssetDatabase.FindAssets(@"VideoGlitchesAbout t:Script", null);
        if (results.Length > 0)
        {
          string assetPath = AssetDatabase.GUIDToAssetPath(results[0]);
          assetPath = assetPath.Replace(@"Scripts/Editor/About/VideoGlitchesAbout.cs", string.Empty);
      ...
      {
        VideoGlitchesAbout window = EditorWindow.GetWindow<VideoGlitchesAbout>(true, @"About 'Video Glitches'", true);
        ...
        GUILayout.BeginVertical(@"box");
        {
          ...
          GUILayout.BeginHorizontal(GUILayout.ExpandWidth(true));
          {
            GUILayout.BeginVertical();
            {
              ...
              if (GUILayout.Button(@" <b>Rate</b> 'Video Glitches', thanks!", buttonStyle, GUILayout.ExpandHeight(true)) == true)
    ...
    public sealed class VintageAboutProcessor : AssetPostprocessor
    {
      static void OnPostprocessAllAssets(string[] importedAssets, string[] deletedAssets, string[] movedAssets, string[] movedFromAssetPaths)
      {
        ...
        {
          if (importedAssets[i].Contains(@"VideoGlitches/Changelog.txt") == true && importedAssets[i].EndsWith(@".meta") == false)
          {
            string assetPath = VideoGlitchesAbout.GetAssetPath();
            if (string.IsNullOrEmpty(assetPath) == false)
            {
              TextAsset changeLogAsset = AssetDatabase.LoadAssetAtPath<TextAsset>(assetPath + @"Changelog.txt");
              if (changeLogAsset != null && string.IsNullOrEmpty(changeLogAsset.text) == false)
              {
                ...
                if (EditorPrefs.GetString(VideoGlitchesAbout.assetID, @"0.0") != version)
                  VideoGlitchesAbout.Open();
